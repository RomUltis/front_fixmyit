...
function afficherErreur(message) {
    ...
    erreurContainer.innerText = message;
    erreurContainer.style.display = "block";
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", function (e) {
        ...
        if (!currentPassword) {
            afficherErreur("Le mot de passe actuel est requis.");
            return;
        }
        if (!pseudo && !newPassword) {
            afficherErreur("Veuillez entrer un nouveau pseudo ou un nouveau mot de passe.");
            return;
        }
        ...
        // Utilisation du proxy pour accéder au backend
        fetch('/profil', {
            method: 'PUT',
            headers: {
        ...
        .then(response => response.json())
        .then(data => {
            console.log('Réponse serveur:', data);  // Debug de la réponse serveur
            if (data.success) {
                alert("Mise à jour réussie !");
                form.reset();
            } else {
                afficherErreur(data.message || "Une erreur est survenue.");
            }
        })
        .catch(error => console.error('Erreur:', error));
    });
